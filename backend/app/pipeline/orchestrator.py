...
class PipelineOrchestrator:
    ...
    async def execute_job(
        ...
        try:
            ...
            logger.warning(f"[Orchestrator] Could not load past topics from DB: {dbe}")
        ...
        for attempt in range(1, max_duplicate_retries + 1):
            try:
                ...
                if attempt > 1:
                    logger.info(
                        f"\n============================================================\n"
                        f"[STAGE: RETRY {attempt}/{max_duplicate_retries}] Immediately Generating Replacement Video (Duplicate Avoidance)\n"
                        f"============================================================"
                    )
                else:
                ...
                logger.warning(
                    ...
                    f"Attempt {attempt}/{max_duplicate_retries}: Video content was duplicate.\n"
